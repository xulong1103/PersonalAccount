...
public class UpdateIncomeActivity extends Activity{
        ...
        save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (name.getText().toString().equals("")) {
                    Toast.makeText(UpdateIncomeActivity.this, "名称不能为空", Toast.LENGTH_LONG).show();
                    return;
                }
                /** 判断金钱是否为空 **/
                if (money.getText().toString().equals("")) {
                    Toast.makeText(UpdateIncomeActivity.this, "金额不能为空", Toast.LENGTH_LONG).show();
                    return;
                }
                /** 将所有信息封装成一个收入对象 **/
                ...
                income.setIncomeId(incomeExtra.getIncomeId());
                /** 新建收入 **/
                boolean flag = accountDB.updateIncome(income);
                /** 判断是否成功 **/
                if (flag) {
                    Toast.makeText(UpdateIncomeActivity.this, "修改成功", Toast.LENGTH_LONG).show();
                    setResult(RESULT_OK);
                    UpdateIncomeActivity.this.finish();
                } else {
                    Toast.makeText(UpdateIncomeActivity.this, "修改失败", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
    ...
    private void initViews() {
        ...
        /** 取得其他页面传入的收入记录 **/
        incomeExtra = (Income) getIntent().getSerializableExtra("income");
        if (incomeExtra != null) {
            /** 将对应属性设置为该收入记录的值 **/
            name.setText(incomeExtra.getName());
            money.setText(String.valueOf(incomeExtra.getMoney()));
            /** 通过适配器与收入记录中的收入类型进行匹配会得到的对应收入类型的下标 **/
            int pos = adapter.getPosition(accountDB.queryIncomeTypeName(incomeExtra.getTypeId()));
            /** 收入类型设置匹配的下标 **/
            type.setSelection(pos);
            /** 日期设置格式为 yyyy-MM-dd 星期* 的格式**/
            date.setText(incomeExtra.getDate().split(" ")[0] + " " + incomeExtra.getDate().split(" ")[1]);
            remark.setText(incomeExtra.getRemark());
        }
    }
